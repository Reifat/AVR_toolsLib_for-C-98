...
template <class T, mbl::size_t N> struct remove_cv<T const [N]> { typedef T type[N]; };
    template <class T, mbl::size_t N> struct remove_cv<T const volatile [N]> { typedef T type[N]; };
    template <class T, mbl::size_t N> struct remove_cv<T volatile [N]> { typedef T type[N]; };
    template <class T> struct remove_cv<T const []> { typedef T type[]; };
    template <class T> struct remove_cv<T const volatile []> { typedef T type[]; };
    template <class T> struct remove_cv<T volatile []> { typedef T type[]; };
